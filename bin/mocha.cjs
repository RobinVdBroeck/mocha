 ...
 * before invoking the "real" executable (`lib/cli/cli.cjs`)
 *
 * @module bin/mocha
 * @private
 */
...
const { loadOptions } = require("../lib/cli/options.cjs");
const {
  unparseNodeFlags,
  isNodeFlag,
  impliesNoTimeouts,
...
const { aliases } = require("../lib/cli/run-option-metadata.cjs");
...
if (mochaArgs["node-option"] || Object.keys(nodeArgs).length || hasInspect) {
  ...
  const mochaPath = require.resolve("../lib/cli/cli.cjs");
  ...
  require("../lib/cli/cli.cjs").main([], mochaArgs);
}
